    ...
    <a href={detail?.url} target="_blank" rel="noreferrer">
      <Box sx={{ width: 350, height: 300, display: "flex", flexDirection: "column" }} className=" dark:bg-white/5 bg-black/5 hover:shadow-[0px_2px_3px_1px] transition duration-300 hover:scale-105 rounded-t-lg">
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", p: 1 }}>
          <Box sx={{ width: 200 }}>
            <CTypography desc={detail?.name} />
          </Box>
          <LazyLoadImage effect="blur" alt={detail.name} src={detail?.image?.thumbnail?.contentUrl || Logo} className="w-[150px] h-[150px] rounded-md object-cover" />
        </Box>
        <Box sx={{ p: 1, maxHeight: 100, display: "flex", flexDirection: "column" }} className=" space-y-1">
          ...
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", py: 1 }}>
            <CTypography className="text-slate-500 text-[12px] " desc={moment(detail.datePublished).fromNow()} />
